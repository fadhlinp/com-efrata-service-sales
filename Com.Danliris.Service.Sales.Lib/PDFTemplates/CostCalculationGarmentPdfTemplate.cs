...
namespace Com.Danliris.Service.Sales.Lib.PDFTemplates
{
    public class CostCalculationGarmentPdfTemplate
    {
        ...
        public MemoryStream GeneratePdfTemplate(CostCalculationGarmentViewModel viewModel, int timeoffset)
        {
            ...
            cell_bottom_column1_1.Phrase = new Phrase("PEMBERI KERJA", normal_font);
            table_bottom_column1_1.AddCell(cell_bottom_column1_1);
            ...
            if (viewModel.Freight == 0) CNF_Price = "IDR";
            ...
            if (viewModel.Insurance == 0) CIF_Price = "IDR";
            ...
            var DESC = (viewModel.CostCalculationGarment_Materials.Any(m => m.isFabricCM) ? "FOB PRICE : RP " + Number.ToRupiahWithoutSymbol(FOB_Remark) + "\n\n" : string.Empty) + fabric;
            ...
            if (isDollar)
            {
                cell_bottom_center.AddElement(new Paragraph($"NOTE: 1 IDR = {Number.ToRupiah(viewModel.Rate.Value)}", normal_font));
            }
